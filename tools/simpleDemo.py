...
import pdb as pdb
namepath = './data/demo/testfiles.txt'
filepath = './data/demo/';
    ...
    for i in range(numFile):
     ...
     #   demo(sess, net, im_name)
        ...
        if (not os.path.exists(txtpath)):
            os.mkdir(txtpath)
        tofile_txt = txtpath + testFiles[i][0:-4] + '.txt'
        tofile_img = imgpath + testFiles[i]
        ftxt = open(tofile_txt,'w')
        im2 = cv2.imread(filepath + im_name)

        for i in range(numBox):
            #ftxt.write(str(res[i][0]) + ',' + str(res[i][1]) + ',' + str(res[i][2]) + ',' + str(res[i][3]) + ',' \
                      # + str(res[i][4])+',' + str(res[i][5]) + '\n')
